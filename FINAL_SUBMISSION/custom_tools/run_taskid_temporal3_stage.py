"""Train and screen a three-frame Task-ID temporal student."""
...
RUN_NAME = "unified_student_taskid_temporal3_seed2025_e4_v1"
...
OUTPUT_ROOT = ROOT / "custom_tools/results/taskid_temporal3_development_v1"
R1_RESULT = (
...
def main():
    ...
    required = [
        CONFIG, INIT, SELECTION, TRAJECTORY_ROOT, RESIDUAL_CONFIG, R1_RESULT,
    ...
    if last.is_file() and resource.is_file():
        print("[REUSE] completed temporal training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial temporal training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    rows = [
        compact(
            "online_r1_control", "online_r1", 2, INIT, R1_RESULT)
    ]
    rows.extend(
        compact(
    ...
    summary = {
        "status": "complete",
        "stage": "three-frame Task-ID temporal observation",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "history": (
            "current DexRep plus two previous 100-D proprioception vectors "
            "and two previous 28-D executed actions"
        ),
        "history_feature_dim": 256,
        "initialization": (
            "exact online-R1 function; new temporal actor columns start at zero"
        ),
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL3_STAGE=COMPLETE", flush=True)
