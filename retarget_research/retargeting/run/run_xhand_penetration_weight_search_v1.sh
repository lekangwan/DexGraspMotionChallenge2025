#!/usr/bin/env bash
# 输入：冻结20轨迹XHand官方基线，以及0/2两个新穿透惩罚权重。
# 输出：两个新细化候选、与当前权重1统一比较的PhysX汇总。
# 内部逻辑：两个候选并行生成，复用已有权重1结果，再顺序物理评测。
# 作用：判断超过-6 mm的深穿透保护是否必要，以及加倍保护会不会妨碍夹持。
...
candidate_root="retarget_research/outputs/xhand_penetration_weight_search_v1"

cd "$project_root" || exit 1

run_candidate() {
  local candidate_name="$1"
  local penetration_weight="$2"
  ...
  "$retarget_python" retarget_research/retargeting/run/run_xhand_contact_manifest.py \
    ...
    --penetration-weight "$penetration_weight" --joint-prior-weight 2 \
...
run_candidate penetration_weight_0 0 & candidate_pid_0=$!
run_candidate penetration_weight_2 2 & candidate_pid_1=$!
candidate_failed=0
wait "$candidate_pid_0" || candidate_failed=1
wait "$candidate_pid_1" || candidate_failed=1
if [[ "$candidate_failed" -ne 0 ]]; then
  echo "至少一个XHand穿透候选失败；修正后重新执行可续跑。" >&2
  exit 1
fi
...
"$retarget_python" retarget_research/retargeting/evaluate/evaluate_candidate_sweep.py \
  --config retarget_research/retargeting/configs/xhand_penetration_weight_search_v1.json \
  --output retarget_research/outputs/xhand_penetration_weight_search_v1_evaluation/search_summary.json \
  --workers 1 --resume
