...
RUNNER=$ROOT/retarget_research/retargeting/run/run_topology_interaction_manifest.py
...
EXP_NAME=${EXP_NAME:-reboot_topology_interaction_cal50_v1}
SURFACE_NEIGHBORS=${SURFACE_NEIGHBORS:-3}
INTERACTION_THRESHOLD=${INTERACTION_THRESHOLD:-0.05}
OPTIMIZE_FROM=${OPTIMIZE_FROM:-close}
...
run_refine() {
  ...
  "$PYTHON" -u "$RUNNER" \
    ...
    --surface-neighbors "$SURFACE_NEIGHBORS" \
    --interaction-threshold "$INTERACTION_THRESHOLD" \
    --optimize-from "$OPTIMIZE_FROM" \
    > "$EXP/logs/${hand}_generate.log" 2>&1
  cp "$BASE/manifests/${hand}_cal50.json" "$EXP/manifests/${hand}_cal50.json"
}

run_refine linker & p1=$!
run_refine xhand & p2=$!
run_refine wuji & p3=$!
status=0
wait "$p1" || status=1
