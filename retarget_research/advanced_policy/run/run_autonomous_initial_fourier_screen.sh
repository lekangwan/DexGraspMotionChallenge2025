#!/usr/bin/env bash
# 三手并行训练傅里叶相位MLP，并在同一50条valid上自主评测。
set -euo pipefail
...
CONFIG="$ROOT/retarget_research/advanced_policy/configs/generated/autonomous_initial_fourier_delta_v1"
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_initial_fourier_delta_v1"
...
for label in linker xhand_official wuji_old; do
  "$PY" "$TRAIN" --config "$CONFIG/${label}_initial_fourier_delta_v1.json" --device cuda \
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_initial_fourier_delta_v1/best.pt" \
    --data-dir "$DATA/$label" --output-dir "$RUN/${label}_initial_fourier_delta_v1/closed_loop_valid50" \
...
for pid in "${pids[@]}"; do wait "$pid"; done
echo "AUTONOMOUS_INITIAL_FOURIER_SCREEN=COMPLETE"
