...
LINKER_TIP_INDICES = {"index": 3, "middle": 6, "ring": 9, "little": 12, "thumb": 14}


METHOD = "anydex_style_segment_vectors_grip_contact_v2"
DEFAULT_VECTOR_CONFIG = RETARGET_ROOT / "configs" / "linker_anydex_vectors_v2.json"
DEFAULT_KEYPOINT_MAP = RETARGET_ROOT / "configs" / "linker_o6_keypoint_map.json"
...
def load_keypoint_map():
    config = json.loads(
        (RETARGET_ROOT / "configs" / "linker_o6_keypoint_map.json").read_text(encoding="utf-8"))
    return config["pairs"]


def linker_points(model, joints, translation, euler, pairs):
    ...
    return linker_world_points(model, pairs)


def zero_pose_points(shadow_model, linker_model, pairs):
    with torch.no_grad():
        shadow = shadow_keypoints(np.zeros((1, 28), dtype=np.float32), shadow_model)[0]
        linker = linker_points(
            linker_model,
            torch.zeros(model_joint_count(linker_model), dtype=torch.float32),
...
def zero_pose_scales(shadow_zero, linker_zero, pairs):
    scales = []
    for item in pairs:
        ...
        target_length = np.linalg.norm(
            linker_zero[int(item["linker_task"])] - linker_zero[int(item["linker_origin"])])
...
class LinkerVectorObjective:
    def __init__(self, model, map_pairs, target_palm, target_vectors, target_directions,
                 pairs, config, previous_values=None, contact_anchors=None,
                 contact_weight=0.0, grip_flexion_weight=0.0,
                 grip_flexion_targets=None, grip_active=False, alphas=None):
        self.model = model
        ...
        self.pos_origin = np.asarray(
            [item["linker_origin"] for item, ok in zip(pairs, position_mask) if ok], dtype=np.int64)
        self.pos_task = np.asarray(
            [item["linker_task"] for item, ok in zip(pairs, position_mask) if ok], dtype=np.int64)
        ...
        self.dir_origin = np.asarray(
            [item["linker_origin"] for item, ok in zip(pairs, direction_mask) if ok], dtype=np.int64)
        self.dir_task = np.asarray(
            [item["linker_task"] for item, ok in zip(pairs, direction_mask) if ok], dtype=np.int64)
    ...
    def __call__(self, values, gradient=None):
        ...
        points = linker_points(self.model, joints, translation, euler, self.map_pairs)
        ...
        if self.contact_anchors is not None and self.contact_weight > 0:
            ...
            anchor_tip_indices = self.contact_anchors["linker_tip_indices"]
        ...
        if self.previous is not None:
            loss = loss + self.previous_weight * torch.mean(
                (joints - self.previous[: self.joint_count]) ** 2)
...
def frame_contact_anchors(frame_points, contact_plan, frame_index):
    ...
    anchor_tip_indices = np.asarray(
        [LINKER_TIP_INDICES[semantics[i]] for i in np.flatnonzero(mask)], dtype=np.int64)
    return {
        "points": torch.as_tensor(anchor_points, dtype=torch.float32),
        "linker_tip_indices": anchor_tip_indices,
    }


def retarget_vector_trajectory(source_points, model, map_pairs, pairs, scales, config,
    ...
    source_tip_indices = list(SOURCE_TIP_INDICES.values())
    ...
    for frame_index, frame_points in enumerate(source_points):
        ...
        if previous is None and initial_values is not None:
            ...
            start = np.zeros(joint_count + 6, dtype=np.float64)
        else:
            start = previous.copy()
        ...
        objective = LinkerVectorObjective(
            model, map_pairs, frame_points[0], target_vectors, target_directions, pairs, config,
...
def retarget_file(args):
    ...
    map_pairs = load_keypoint_map()
    map_pair_count = len(map_pairs)
    for item in pairs:
        for key in ("linker_origin", "linker_task"):
            if not 0 <= int(item[key]) < map_pair_count:
                raise ValueError(f"Linker map索引越界: {item['semantic']}")
    shadow_model = build_shadow_model()
    linker_model = build_linker_model(joint_mode="coupled6")
    shadow_zero, linker_zero = zero_pose_points(shadow_model, linker_model, map_pairs)
    scales = zero_pose_scales(shadow_zero, linker_zero, pairs)
    joint_lower = linker_model.revolute_joints_q_lower[0].detach().cpu().numpy()
    joint_upper = linker_model.revolute_joints_q_upper[0].detach().cpu().numpy()
    joint_count = model_joint_count(linker_model)
    object_name = args.object_name or args.source.stem
    warm_data = None
    warm_by_index = {}
    if args.warm_start_target is not None:
        warm_data = np.load(args.warm_start_target, allow_pickle=True).item()
        warm_by_index = {
            int(index): frames
            for index, frames in zip(
                warm_data["source_trajectory_indices"], warm_data["grasp_seqs"]
            )
        }
    outputs, all_losses = [], []
    for trajectory_number, source_index in enumerate(indices):
        ...
        internal, losses = retarget_vector_trajectory(
            source_points, linker_model, map_pairs, pairs, scales, vector_config,
            ...
            initial_values=(
                np.concatenate([
                    warm_by_index[source_index][0, 6:],
                    warm_by_index[source_index][0, :6],
                ])
                if source_index in warm_by_index else None
            ),
        )
        saved = np.concatenate(
            [internal[:, joint_count: joint_count + 6], internal[:, :joint_count]], axis=1)
    ...
    output = {
        ...
        "mapping_semantics": [item["semantic"] for item in map_pairs],
        ...
        "contact_fallback": args.contact_fallback,
        "warm_start_target": (
            str(args.warm_start_target.resolve())
            if args.warm_start_target is not None else None
        ),
    }
...
def main():
    ...
    parser.add_argument("--warm-start-target", type=Path)
