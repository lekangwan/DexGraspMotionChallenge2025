#!/usr/bin/env bash
# 三手局部线性回归第二轮：缩小邻域，减少不同抓形被过度平均。
set -euo pipefail

ROOT="/home/lekangwan/projects/DexGraspMotionChallenge2025-final-release"
PY="/home/lekangwan/miniconda3/envs/hand-retarget/bin/python"
BUILD="$ROOT/retarget_research/advanced_policy/prepare/build_trajectory_local_ridge.py"
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_local_ridge_k15_v1"

cd "$ROOT"
for spec in "linker:10" "xhand_official:1" "wuji_old:10"; do
  label="${spec%%:*}"
  ridge="${spec##*:}"
  output="$RUN/${label}_local_ridge_k15_v1/best.pt"
  if [[ ! -f "$output" ]]; then
    "$PY" "$BUILD" --data-dir "$DATA/$label" --output "$output" \
      --local-k 15 --ridge "$ridge"
  fi
done
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_local_ridge_k15_v1/best.pt" --data-dir "$DATA/$label" \
    --output-dir "$RUN/${label}_local_ridge_k15_v1/closed_loop_valid50" \
...
echo "AUTONOMOUS_LOCAL_RIDGE_K15_SCREEN=COMPLETE"
