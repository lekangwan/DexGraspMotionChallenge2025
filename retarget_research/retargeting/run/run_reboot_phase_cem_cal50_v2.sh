...
EXP_NAME=${EXP_NAME:-reboot_phase_cem_cal50_v2}
PARAMETERIZATION=${PARAMETERIZATION:-phase}
POPULATION=${POPULATION:-8}
ELITE=${ELITE:-2}
ITERATIONS=${ITERATIONS:-2}
CONFIRM_SINGLE_ENV=${CONFIRM_SINGLE_ENV:-0}
PARAMETER_SCALE=${PARAMETER_SCALE:-1.0}
SCORE_MODE=${SCORE_MODE:-standard}
EXP=$ROOT/retarget_research/outputs/$EXP_NAME
LINKER_BASE=${LINKER_BASE:-$ROOT/retarget_research/outputs/reboot_physics_cem_transport_cal50_v2}
OTHER_BASE=${OTHER_BASE:-$ROOT/retarget_research/outputs/reboot_physics_cem_cal50_v1}
...
run_cem() {
  ...
  local confirm_args=()
  if [[ "$CONFIRM_SINGLE_ENV" == "1" ]]; then
    confirm_args+=(--confirm-single-env)
  fi
  "$PYTHON" -u "$RUNNER" \
    --hand "$hand" --manifest "$base/manifests/${hand}_cal50.json" \
    --target-dir "$base/independent_targets/$hand" \
    --output-dir "$EXP/raw/$hand" --population "$POPULATION" \
    --elite "$ELITE" --iterations "$ITERATIONS" \
    --parameterization "$PARAMETERIZATION" \
    --parameter-scale "$PARAMETER_SCALE" \
    --score-mode "$SCORE_MODE" \
    --selection-margin 1.0 --seed 20260828 --device cpu \
    "${confirm_args[@]}" \
