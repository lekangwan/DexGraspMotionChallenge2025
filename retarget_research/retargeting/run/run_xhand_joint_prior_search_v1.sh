#!/usr/bin/env bash
# 输入：冻结20轨迹XHand官方基线，以及0/4两个新关节先验权重。
# 输出：两个新细化候选、与当前权重2统一比较的PhysX汇总。
# 内部逻辑：两个候选并行生成；复用穿透权重2且先验2的候选，再顺序物理评测。
# 作用：决定接触细化可以自由改变抓形，还是应更强地留在官方姿态附近。
...
candidate_root="retarget_research/outputs/xhand_joint_prior_search_v1"

cd "$project_root" || exit 1

run_candidate() {
  local candidate_name="$1"
  local joint_prior_weight="$2"
  ...
  "$retarget_python" retarget_research/retargeting/run/run_xhand_contact_manifest.py \
    ...
    --penetration-weight 2 --joint-prior-weight "$joint_prior_weight" \
...
run_candidate joint_prior_0 0 & candidate_pid_0=$!
run_candidate joint_prior_4 4 & candidate_pid_1=$!
candidate_failed=0
wait "$candidate_pid_0" || candidate_failed=1
wait "$candidate_pid_1" || candidate_failed=1
if [[ "$candidate_failed" -ne 0 ]]; then
  echo "至少一个XHand关节先验候选失败；修正后重新执行可续跑。" >&2
  exit 1
fi
...
"$retarget_python" retarget_research/retargeting/evaluate/evaluate_candidate_sweep.py \
  --config retarget_research/retargeting/configs/xhand_joint_prior_search_v1.json \
  --output retarget_research/outputs/xhand_joint_prior_search_v1_evaluation/search_summary.json \
  --workers 1 --resume
