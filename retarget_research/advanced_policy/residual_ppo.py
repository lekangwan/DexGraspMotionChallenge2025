...
def ppo_update(model, optimizer, batch, config):
    ...
    for _ in range(config.update_epochs):
        ...
        for start in range(0, batch_size, minibatch_size):
            ...
            actor_obs = batch["actor_obs"][indices]
            critic_obs = batch["critic_obs"][indices]
            ...
            if anchor_mask is not None:
                ...
                if minibatch_anchor_mask.any():
                    mean_action = torch.tanh(model.distribution(
                        actor_obs[minibatch_anchor_mask]).mean)
            ...
            with torch.no_grad():
                post_log_prob, _, _ = model.evaluate(
                    actor_obs, critic_obs, actions)
                post_ratio = (post_log_prob - old_log_probs).exp()
                post_kl = ((post_ratio - 1.0)
                           - (post_log_prob - old_log_probs)).mean()
                totals["policy_loss"] += policy_loss.item()
                totals["value_loss"] += value_loss.item()
                totals["entropy"] += entropy_mean.item()
                totals["approx_kl"] += post_kl.item()
                totals["clip_fraction"] += (
                    (post_ratio - 1.0).abs() > config.clip_ratio).float().mean().item()
                ...
                updates += 1
                if post_kl.item() > config.target_kl:
                    early_stopped = True
                    early_stop_kl = max(
                        early_stop_kl, float(post_kl.item()))
                    break
        if early_stopped:
            break
