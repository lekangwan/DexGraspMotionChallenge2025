...
EXP=$ROOT/retarget_research/outputs/reboot_geort_canonical_v2
...
for hand in linker xhand wuji; do
  echo "===== 训练 $hand GeoRT canonical exact-FK ====="
  ...
  "$PYTHON" -u "$METHOD" train \
    ...
    --device cuda --workspace-samples 20000 --epoch-samples 20000 \
    --epochs 50 --batch-size 256 --hidden 128 --learning-rate 0.0001
  ...
  "$PYTHON" -u "$METHOD" apply \
    ...
    --audit "$OLD_AUDIT/${hand}_stable_audit.json" \
    --checkpoint "$EXP/checkpoints/${hand}.pt" \
    --device cuda --split calibration --one-per-category \
...
run_eval() {
  ...
  "$PYTHON" -u "$EVALUATOR" \
    ...
    --steps-per-frame 3 --hold-steps 30 2>&1 | sed "s/^/[$hand] /"
}
