"""Train and screen Temporal3 with future-three-action auxiliary prediction."""
...
CONFIG = (
    ...
    / "unified_student_taskid_temporal3_chunk4aux_v1.yaml"
)
INIT = (
...
RUN_NAME = "unified_student_taskid_temporal3_chunk4aux_seed2025_e4_v1"
...
OUTPUT_ROOT = (
    ROOT / "custom_tools/results/taskid_temporal3_chunk4aux_development_v1"
)
CONTROL_CHECKPOINT = (
    ROOT / "custom_tools/runs/bc/"
...
def evaluate(cli, epoch):
    ...
    output = OUTPUT_ROOT / "chunk4aux_epoch{:02d}.yaml".format(epoch)
    if output.is_file():
        result(output, model)
        print("[REUSE] chunk4aux epoch {}".format(epoch), flush=True)
        return output
    run([
        sys.executable,
        "-u",
...
def main():
    ...
    required = [
        ...
        CONTROL_CHECKPOINT, CONTROL_RESULT,
    ...
    if missing:
        raise FileNotFoundError(
            "Missing action-chunk inputs: {}".format(missing))

    last = RUN_DIR / "last.ckpt"
    resource = RUN_DIR / "resource_summary.yaml"
    if last.is_file() and resource.is_file():
        print("[REUSE] completed chunk4aux training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial chunk4aux training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    if cli.dry_run:
        print(
            "DRY_RUN: four chunk4aux checkpoints will be evaluated on the "
    ...
    control = compact(
        "temporal3_control", "temporal3", 4,
        CONTROL_CHECKPOINT, CONTROL_RESULT)
    candidates = [
        compact(
            "chunk4aux_epoch{:02d}".format(epoch),
            "temporal3_future_action_auxiliary",
            epoch,
            checkpoint(epoch),
            outputs[epoch])
        for epoch in EPOCHS
    ...
    summary = {
        "status": "complete",
        "stage": "Temporal3 future-three-action auxiliary prediction",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "control": control,
        "candidate_ranking": candidates,
        "controlled_difference": (
            "same R1 initialization, Temporal3 inputs, datasets, target, "
            "sampling, learning rate, seed, and four epochs; candidate adds "
            "a future-three-action auxiliary loss with weight 0.25 only"),
        "execution": (
            "closed loop; only the current 28-D action is executed and future "
            "predictions are discarded at inference"),
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL3_CHUNK4AUX_STAGE=COMPLETE", flush=True)
    print(
