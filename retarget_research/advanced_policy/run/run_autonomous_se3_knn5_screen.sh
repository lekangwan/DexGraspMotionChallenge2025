#!/usr/bin/env bash
# 三手SE(3)对齐的5NN整段轨迹迁移，同一50条valid screen。
set -euo pipefail
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_se3_knn5_v1"

cd "$ROOT"
for label in linker xhand_official wuji_old; do
  output="$RUN/${label}_se3_knn5_v1/best.pt"
  if [[ ! -f "$output" ]]; then
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_se3_knn5_v1/best.pt" --data-dir "$DATA/$label" \
    --output-dir "$RUN/${label}_se3_knn5_v1/closed_loop_valid50" \
...
echo "AUTONOMOUS_SE3_KNN5_SCREEN=COMPLETE"
