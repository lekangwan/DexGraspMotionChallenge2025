#!/usr/bin/env bash
# 三手并行训练纯参数PCA轨迹MLP，并在同一50条valid上screen。
set -euo pipefail

ROOT="/home/lekangwan/projects/DexGraspMotionChallenge2025-final-release"
PY="/home/lekangwan/miniconda3/envs/hand-retarget/bin/python"
TRAIN="$ROOT/retarget_research/advanced_policy/train_trajectory_pca_mlp.py"
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_pca_mlp_v1"

cd "$ROOT"
train_hand() {
  local label="$1"
  "$PY" "$TRAIN" --data-dir "$DATA/$label" --output-dir "$RUN/${label}_pca_mlp_v1" \
    --rank 32 --epochs 400 --device cuda
}
pids=()
for label in linker xhand_official wuji_old; do
  train_hand "$label" 2>&1 | sed -u "s/^/[train-$label] /" & pids+=("$!")
done
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_pca_mlp_v1/best.pt" --data-dir "$DATA/$label" \
    --output-dir "$RUN/${label}_pca_mlp_v1/closed_loop_valid50" \
...
for pid in "${pids[@]}"; do wait "$pid"; done
echo "AUTONOMOUS_PCA_MLP_SCREEN=COMPLETE"
