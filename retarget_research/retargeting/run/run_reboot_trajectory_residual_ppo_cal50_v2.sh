...
EXP=$ROOT/retarget_research/outputs/reboot_trajectory_residual_ppo_phase_cal50_v2
...
train_hand() {
  ...
  echo "[$hand] 开始phase-gated残差PPO训练：60轮"
  ...
  "$PYTHON" -u "$TRAINER" \
    ...
    --run-dir "$EXP/runs/$hand" --iterations 60 --num-envs 50 \
    --residual-scale 0.20 --gate-mode closure --evaluation-interval 10 \
    --selection-rollouts 2 --export-rollouts 3 \
    --device cuda --seed 20260830 \
...
wait "$p3" || status=1
if [[ "$status" -ne 0 ]]; then
  for hand in linker xhand wuji; do tail -n 40 "$EXP/logs/${hand}_train.log" 2>/dev/null || true; done
  exit "$status"
fi
echo "训练完成，开始独立回放。"
...
wait "$p3" || status=1
if [[ "$status" -ne 0 ]]; then
  for hand in linker xhand wuji; do tail -n 40 "$EXP/logs/${hand}_evaluate.log" 2>/dev/null || true; done
  exit "$status"
fi
...
"$PYTHON" -u "$AUDITOR" --config "$PROTOCOL" \
  ...
  --output-dir "$EXP/stable_audit_v3" > "$EXP/logs/audit.log" 2>&1

"$PYTHON" - <<PY
import json
from pathlib import Path
d = json.loads(Path("$EXP/stable_audit_v3/three_hand_stable_audit_summary.json").read_text())
for hand in ("linker", "xhand", "wuji"):
    print(hand, "stable", d[hand]["stable_physics_success_count"],
          "transport", d[hand]["transport_quality_success_count"])
PY
