...
- Linker每条轨迹仍只有一个12维候选（腕部6维+主动手指6维）：所有轨迹使用同一组有符号关节残差；重放前只根据`obj_scale`和抬升首帧关节标准差确定PD增益，不按物理成功结果取候选并集。冻结参数见`retargeting/configs/linker_o6_adaptive_v2.json`。
- Linker独立验证已完成：10个全新物体×2条上，几何基线4/20、统一夹紧7/20、固定残差6/20、自适应PD仍6/20。正式主方法据此选择更简单的统一夹紧v1和默认PD；v2的开发集5/10保留为过拟合边界案例。
- 同一独立20条上，XHand官方基线14/20、真实指腹细化16/20；Wuji v1/v2分别16/20和12/20。当前三手正式主候选冻结为Linker夹紧7/20、XHand指腹16/20和Wuji v1 16/20。
...
- seed 20260808已随机冻结50类、100物体、1000轨迹，其中每物体2条calibration、8条heldout；三手正式候选生成已启动，结果目录不进入Git。
