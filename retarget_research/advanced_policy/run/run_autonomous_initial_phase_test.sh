#!/usr/bin/env bash
# 冻结三个valid已选模型，并行评测500条对象隔离test；测试时禁止任何专家未来信息。
set -euo pipefail
...
run_hand() {
  ...
  "$PY" "$EVAL" \
    --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" --split test \
    --target-dir "$target" --checkpoint "$RUN_ROOT/$experiment/best.pt" \
    --data-dir "$DATA_ROOT/$label" --output-dir "$RUN_ROOT/$experiment/closed_loop_test" \
...
"$PY" "$ROOT/retarget_research/advanced_policy/summarize_autonomous_initial_phase.py" \
  --run-root "$RUN_ROOT" --split test
echo "AUTONOMOUS_INITIAL_PHASE_TEST=COMPLETE"
