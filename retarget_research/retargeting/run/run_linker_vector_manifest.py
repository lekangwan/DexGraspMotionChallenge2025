...
from retarget_linker_vectors import METHOD


RUN_DIR = Path(__file__).resolve().parent
RETARGET_ROOT = RUN_DIR.parent
VECTOR_SCRIPT = RUN_DIR / "retarget_linker_vectors.py"
...
def build_command(entry, source, output, args):
    ...
    if args.warm_start_dir is not None:
        command.extend([
            "--warm-start-target",
            str((args.warm_start_dir / f"{entry['object_name']}.npy").resolve()),
        ])
    command.extend(["--contact-fallback", args.contact_fallback])
    return command
...
def existing_output_matches(output, entry, args):
    ...
    try:
        ...
        return bool(
            ...
            == (len(entry["trajectory_indices"]), 70, 12)
            ...
            and (
                (args.warm_start_dir is None and data.get("warm_start_target") is None)
                or (
                    args.warm_start_dir is not None
                    and Path(str(data.get("warm_start_target"))).resolve()
                    == (args.warm_start_dir / f"{entry['object_name']}.npy").resolve()
                )
            )
        )
