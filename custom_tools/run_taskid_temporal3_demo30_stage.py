"""Train and screen Temporal3 with a 70:30 teacher/demo offline target."""
...
CONFIG = (
    ...
    / "unified_student_taskid_temporal3_demo30_v1.yaml"
)
INIT = (
...
RUN_NAME = "unified_student_taskid_temporal3_demo30_seed2025_e4_v1"
...
OUTPUT_ROOT = (
    ROOT / "custom_tools/results/taskid_temporal3_demo30_development_v1"
)
CONTROL_CHECKPOINT = (
    ROOT / "custom_tools/runs/bc/"
...
def evaluate(cli, epoch):
    ...
    output = OUTPUT_ROOT / "demo30_epoch{:02d}.yaml".format(epoch)
    if output.is_file():
        result(output, model)
        print("[REUSE] demo30 epoch {}".format(epoch), flush=True)
        return output
    run([
        sys.executable,
        "-u",
...
def main():
    ...
    if missing:
        raise FileNotFoundError(
            "Missing Temporal3 demo-anchor inputs: {}".format(missing))

    last = RUN_DIR / "last.ckpt"
    resource = RUN_DIR / "resource_summary.yaml"
    if last.is_file() and resource.is_file():
        print("[REUSE] completed demo30 training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial demo30 training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    if cli.dry_run:
        print(
            "DRY_RUN: four demo-anchored Temporal3 checkpoints will be "
            "evaluated on the frozen 12-object development split.",
            flush=True)
        return
    ...
    control = compact(
        "temporal3_teacher100_control", "temporal3_teacher100", 4,
        CONTROL_CHECKPOINT, CONTROL_RESULT)
    candidates = [
        compact(
            "temporal3_demo30_epoch{:02d}".format(epoch),
            "temporal3_teacher70_demo30",
            epoch,
            checkpoint(epoch),
            outputs[epoch])
        for epoch in EPOCHS
    ...
    summary = {
        "status": "complete",
        "stage": "Temporal3 successful-demonstration anchor",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "control": control,
        "candidate_ranking": candidates,
        "controlled_difference": (
            "same R1 initialization, architecture, offline/online datasets, "
            "sampling, learning rate, seed, and four epochs; offline target "
            "changes from teacher100 to teacher70_demo30 only"),
        "online_target": (
            "100% teacher because online actions and teacher_actions are equal"),
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL3_DEMO30_STAGE=COMPLETE", flush=True)
    print(
