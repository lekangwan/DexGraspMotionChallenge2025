"""Train and screen a seven-frame Task-ID temporal student."""
...
CONFIG = (
    ...
    / "unified_student_taskid_temporal7_v1.yaml"
)
INIT = (
...
RUN_NAME = "unified_student_taskid_temporal7_seed2025_e4_v1"
...
OUTPUT_ROOT = ROOT / "custom_tools/results/taskid_temporal7_development_v1"
CONTROL_CHECKPOINT = (
    ROOT / "custom_tools/runs/bc/"
...
def evaluate(cli, epoch):
    ...
    output = OUTPUT_ROOT / "temporal7_epoch{:02d}.yaml".format(epoch)
    if output.is_file():
        result(output, model)
        print("[REUSE] temporal7 epoch {}".format(epoch), flush=True)
        return output
    run([
        sys.executable,
        "-u",
...
def main():
    ...
    if missing:
        raise FileNotFoundError(
            "Missing Temporal7 inputs: {}".format(missing))

    last = RUN_DIR / "last.ckpt"
    resource = RUN_DIR / "resource_summary.yaml"
    if last.is_file() and resource.is_file():
        print("[REUSE] completed Temporal7 training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial Temporal7 training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    if cli.dry_run:
        print(
            "DRY_RUN: four Temporal7 checkpoints will be evaluated on the "
    ...
    candidates = [
        compact(
            "temporal7_epoch{:02d}".format(epoch),
            "temporal7",
            epoch,
            checkpoint(epoch),
            outputs[epoch])
    ...
    summary = {
        "status": "complete",
        "stage": "seven-frame Task-ID temporal observation",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "control": control,
        "candidate_ranking": candidates,
        "controlled_difference": (
            "same R1 initialization, architecture family, datasets, target, "
            "sampling, learning rate, seed, and four epochs; history changes "
            "from 3 contiguous frames to 7 contiguous frames only"),
        "history_span_at_60hz": {
            "temporal3_seconds": 2.0 / 60.0,
            "temporal7_seconds": 6.0 / 60.0,
        },
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL7_STAGE=COMPLETE", flush=True)
    print(
