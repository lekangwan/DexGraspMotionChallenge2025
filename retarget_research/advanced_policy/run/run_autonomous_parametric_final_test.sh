#!/usr/bin/env bash
# 冻结后的三手纯参数策略，在500条对象隔离test上各评测一次。
set -euo pipefail
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_parametric_final_test_v1"
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" \
    ...
    --output-dir "$RUN/$tag" --device cuda --workers 2 \
...
evaluate xhand xhand_official xhand \
  "$ROOT/retarget_research/advanced_policy/runs/autonomous_initial_phase_huber_v1/xhand_official_initial_phase_huber_v1/best.pt" \
...
echo "AUTONOMOUS_PARAMETRIC_FINAL_TEST=COMPLETE"
