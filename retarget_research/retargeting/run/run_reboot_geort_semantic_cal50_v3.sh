...
V2=$ROOT/retarget_research/outputs/reboot_geort_canonical_v2
EXP=$ROOT/retarget_research/outputs/reboot_geort_semantic_v3

mkdir -p "$EXP/checkpoints" "$EXP/targets" "$EXP/manifests" "$EXP/evaluation" "$EXP/traces"
cd "$ROOT"

for hand in linker xhand wuji; do
  echo "===== 微调 $hand GeoRT + 语义指尖锚 ====="
  MPLCONFIGDIR=/tmp/matplotlib-retarget OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 \
  "$PYTHON" -u "$METHOD" train \
    --hand "$hand" --audit "$OLD_AUDIT/${hand}_stable_audit.json" \
    --source-cache "$V2/cache/${hand}_source_tips.npy" \
    --init-checkpoint "$V2/checkpoints/${hand}.pt" \
    --checkpoint "$EXP/checkpoints/${hand}.pt" --device cuda \
    --workspace-samples 20000 --epoch-samples 20000 --epochs 30 --batch-size 256 \
    --hidden 128 --learning-rate 0.0001 --semantic-weight 200

  "$PYTHON" -u "$METHOD" apply \
    --hand "$hand" --audit "$OLD_AUDIT/${hand}_stable_audit.json" \
    --checkpoint "$EXP/checkpoints/${hand}.pt" --device cuda \
    --split calibration --one-per-category --formal-manifest "$FORMAL" \
...
run_eval() {
  ...
  "$PYTHON" -u "$EVALUATOR" --hand "$hand" \
    --manifest "$EXP/manifests/${hand}_cal50.json" --target-dir "$EXP/targets/$hand" \
    --output-dir "$EXP/evaluation/$hand" --policy-trace-dir "$EXP/traces/$hand" \
