#!/usr/bin/env bash
# 三手局部线性整段轨迹回归，同一每类1条的50 valid screen。
set -euo pipefail
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_local_ridge_v1"

cd "$ROOT"
for label in linker xhand_official wuji_old; do
  output="$RUN/${label}_local_ridge_v1/best.pt"
  if [[ ! -f "$output" ]]; then
    "$PY" retarget_research/advanced_policy/prepare/build_trajectory_local_ridge.py \
      --data-dir "$DATA/$label" --output "$output"
  fi
done
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_local_ridge_v1/best.pt" --data-dir "$DATA/$label" \
    --output-dir "$RUN/${label}_local_ridge_v1/closed_loop_valid50" \
...
echo "AUTONOMOUS_LOCAL_RIDGE_SCREEN=COMPLETE"
