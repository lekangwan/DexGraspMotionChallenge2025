"""Train and screen five multiscale observations with lags [6,3,2,1]."""
...
CONFIG = (
    ...
    / "unified_student_taskid_temporal5_multiscale6321_v1.yaml"
)
INIT = (
...
RUN_NAME = "unified_student_taskid_temporal5_multiscale6321_seed2025_e4_v1"
...
OUTPUT_ROOT = (
    ROOT / "custom_tools/results/taskid_temporal5_multiscale_development_v1"
)
CONTROL_CHECKPOINT = (
    ROOT / "custom_tools/runs/bc/"
...
def evaluate(cli, epoch):
    ...
    output = OUTPUT_ROOT / "multiscale_epoch{:02d}.yaml".format(epoch)
    if output.is_file():
        result(output, model)
        print("[REUSE] multiscale epoch {}".format(epoch), flush=True)
        return output
    run([
        sys.executable,
        "-u",
...
def main():
    ...
    if missing:
        raise FileNotFoundError(
            "Missing multiscale-history inputs: {}".format(missing))

    last = RUN_DIR / "last.ckpt"
    resource = RUN_DIR / "resource_summary.yaml"
    if last.is_file() and resource.is_file():
        print("[REUSE] completed multiscale training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial multiscale training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    if cli.dry_run:
        print(
            "DRY_RUN: four multiscale checkpoints will be evaluated on the "
    ...
    control = compact(
        "temporal3_control", "temporal3_contiguous", 4,
        CONTROL_CHECKPOINT, CONTROL_RESULT)
    candidates = [
        compact(
            "multiscale_epoch{:02d}".format(epoch),
            "temporal5_multiscale_lags_6_3_2_1",
            epoch,
            checkpoint(epoch),
            outputs[epoch])
        for epoch in EPOCHS
    ...
    summary = {
        "status": "complete",
        "stage": "five-frame multiscale Task-ID temporal observation",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "control": control,
        "candidate_ranking": candidates,
        "controlled_difference": (
            "same R1 initialization, datasets, target, sampling, learning "
            "rate, seed, and four epochs; candidate supplies history lags "
            "[6,3,2,1] instead of contiguous [2,1]"),
        "history_span_at_60hz": {
            "contiguous_seconds": 2.0 / 60.0,
            "multiscale_seconds": 6.0 / 60.0,
        },
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL5_MULTISCALE_STAGE=COMPLETE", flush=True)
    print(
