#!/usr/bin/env bash
# 手腕用5NN平滑，手指复制1NN成功抓形。
set -euo pipefail

ROOT="/home/lekangwan/projects/DexGraspMotionChallenge2025-final-release"
PY="/home/lekangwan/miniconda3/envs/hand-retarget/bin/python"
BUILD="$ROOT/retarget_research/advanced_policy/prepare/build_trajectory_retrieval.py"
...
RUN="$ROOT/retarget_research/advanced_policy/runs/autonomous_wrist5_finger1_v1"

cd "$ROOT"
for label in linker xhand_official wuji_old; do
  output="$RUN/${label}_wrist5_finger1_v1/best.pt"
  if [[ ! -f "$output" ]]; then
    "$PY" "$BUILD" --data-dir "$DATA/$label" --output "$output" \
      --k 5 --finger-k 1 --features wrist
  fi
done
...
evaluate() {
  ...
  "$PY" "$EVAL" --hand "$hand" --manifest "$MANIFEST" --policy-split "$SPLIT" \
    ...
    --checkpoint "$RUN/${label}_wrist5_finger1_v1/best.pt" --data-dir "$DATA/$label" \
    --output-dir "$RUN/${label}_wrist5_finger1_v1/closed_loop_valid50" \
...
echo "AUTONOMOUS_WRIST5_FINGER1_SCREEN=COMPLETE"
