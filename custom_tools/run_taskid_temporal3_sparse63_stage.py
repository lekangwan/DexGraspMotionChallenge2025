"""Train and screen three sparse observations spanning about 100 ms."""
...
CONFIG = (
    ...
    / "unified_student_taskid_temporal3_sparse63_v1.yaml"
)
INIT = (
...
RUN_NAME = "unified_student_taskid_temporal3_sparse63_seed2025_e4_v1"
...
OUTPUT_ROOT = (
    ROOT / "custom_tools/results/taskid_temporal3_sparse63_development_v1"
)
CONTROL_CHECKPOINT = (
    ROOT / "custom_tools/runs/bc/"
...
def evaluate(cli, epoch):
    ...
    output = OUTPUT_ROOT / "sparse63_epoch{:02d}.yaml".format(epoch)
    if output.is_file():
        result(output, model)
        print("[REUSE] sparse63 epoch {}".format(epoch), flush=True)
        return output
    run([
        sys.executable,
        "-u",
...
def main():
    ...
    if missing:
        raise FileNotFoundError(
            "Missing sparse-history inputs: {}".format(missing))

    last = RUN_DIR / "last.ckpt"
    resource = RUN_DIR / "resource_summary.yaml"
    if last.is_file() and resource.is_file():
        print("[REUSE] completed sparse-history training: {}".format(
            RUN_DIR), flush=True)
    else:
        existing = list(RUN_DIR.glob("*.ckpt"))
        if existing:
            raise RuntimeError(
                "Partial sparse-history training needs inspection: {}".format(
                    RUN_DIR))
        run([
            sys.executable,
            "-u",
    ...
    if cli.dry_run:
        print(
            "DRY_RUN: four sparse-history checkpoints will be evaluated on "
    ...
    candidates = [
        compact(
            "sparse63_epoch{:02d}".format(epoch),
            "temporal3_sparse_lags_6_3",
            epoch,
            checkpoint(epoch),
            outputs[epoch])
        for epoch in EPOCHS
    ...
    summary = {
        "status": "complete",
        "stage": "sparse three-frame Task-ID temporal observation",
        "formal_final_holdout_result": False,
        "final_holdout_accessed": False,
        "control": control,
        "candidate_ranking": candidates,
        "controlled_difference": (
            "same R1 initialization, input dimension, datasets, target, "
            "sampling, learning rate, seed, and four epochs; history changes "
            "from contiguous lags [2,1] to sparse lags [6,3] only"),
        "history_span_at_60hz": {
            "contiguous_seconds": 2.0 / 60.0,
            "sparse_seconds": 6.0 / 60.0,
        },
        "selection_metric": (
    ...
    print("TASKID_TEMPORAL3_SPARSE63_STAGE=COMPLETE", flush=True)
    print(
